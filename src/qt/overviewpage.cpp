...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAliothAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAliothAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAmountAndRounds = strAnonymizeAliothAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAliothAmount*COIN) nMaxToAnonymize = nAnonymizeAliothAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAliothAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeAliothAmount));
        strAmountAndRounds = strAnonymizeAliothAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
