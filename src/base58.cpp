...
namespace {
    class CAliothAddressVisitor : public boost::static_visitor<bool> {
    private:
        CAliothAddress *addr;
    public:
        CAliothAddressVisitor(CAliothAddress *addrIn) : addr(addrIn) { }
...
bool CAliothAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CAliothAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CAliothAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CAliothAddressVisitor(this), dest);
}

bool CAliothAddress::IsValid() const {
...
CTxDestination CAliothAddress::Get() const {
    if (!IsValid())
...
bool CAliothAddress::GetKeyID(CKeyID &keyID) const {
...
bool CAliothAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CAliothSecret::SetKey(const CKey& vchSecret) {
...
CKey CAliothSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CAliothSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CAliothSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CAliothSecret::SetString(const std::string& strSecret) {
