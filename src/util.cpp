...
int nAnonymizeAliothAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Alioth
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Alioth
    // Mac: ~/Library/Application Support/Alioth
    // Unix: ~/.Alioth
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Alioth";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Alioth.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "aliothd.pid"));
